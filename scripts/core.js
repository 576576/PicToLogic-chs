...
core.build = () => {
	...
	d.cont.pane(t => {
		...
		t.button("更改屏幕类型", icon, () => {
			ui.select("选择屏幕", displays, d => {
				core.display = d;
		...
		t.check("使用灰度透明", core.useGray, b => {core.useGray = b})
			.growX().center().row();

		const quality = new Table();
		quality.add("图片质量:").center().row();
		...
		quality.row();
		quality.check("使用HSV模型", core.hsv, b => {core.hsv = b})
